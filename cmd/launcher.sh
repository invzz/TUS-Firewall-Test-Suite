...
select_env_and_run_game() {
    ...
    case $env_choice in
        1)
            ...
            docker-compose --env-file configs/environments/.env.light -f configs/docker/docker-compose-game.yml up --build
            ;;
        2)
            echo "Starting normal testing configuration..."
            docker-compose --env-file configs/environments/.env.normal -f configs/docker/docker-compose-game.yml up --build
            ;;
        3)
            echo "Starting stress testing configuration..."
            docker-compose --env-file configs/environments/.env.stress -f configs/docker/docker-compose-game.yml up --build
            ;;
        4)
            ...
            if [[ $confirm =~ ^[Yy]$ ]]; then
                docker-compose --env-file configs/environments/.env.performance -f configs/docker/docker-compose-game.yml up --build
            else
        ...
        5)
            ...
            docker-compose --env-file configs/environments/.env.ut-specs -f configs/docker/docker-compose-game.yml up --build
            ;;
        6)
...
case $choice in
    ...
    4)
        ...
        KEEP_RUNNING=true docker-compose -f configs/docker/docker-compose.yml up --build
        ;;
    5)
